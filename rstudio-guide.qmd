...
![Quickly view data frames in a spreadsheet-like environment by running `View(data_name)` or clicking on a data frame object in the environment pane on the right.](./files/dataviewer.png){#fig-dataviewer fig-alt="A screen shot of RStudio with a view of rectangular data on the left and the environment panel on the right."}
...
![RStudio Global Options: Set these options for the Workspace.](./files/rstudio-workspace.png){#fig-workspace width=50% fig-alt="A screen shot of the Settings window for RStudio."}

These settings make sure that your enviornment is cleared everytime you exit RStudio. This might seem like it is painful, but it is short-term pain for long-term gain. It will teach you to keep what you want to save in your R script so that you can always recreate objects or rerun analysis later. The *true source* for the analysis is your script; enviornmental objects can be recreated through the script. It is valuable to check this every once in a while by restarting R. You can then rerun your entire script to make sure that you have fully captured your analysis in the script.
